...
public class HRCListReportServlet extends HttpServlet {
	private String HRCRoleId = "3";
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String contentPage = "/jsp/hrcInitRepList.jsp";
		...
		authorityMapping.setRoleId(HRCRoleId); //super admin authority
		try {
		...
		//set page
		try {
			reportamount = DAOFactory.getIReportDAOInstance().getAmountHRC(); //get amount of records user have	
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		//set report
		try {
			backinfo = DAOFactory.getIReportDAOInstance().HRCListReport(page);	
		} catch (Exception e) {
			e.printStackTrace();
		}
		//jump to page
