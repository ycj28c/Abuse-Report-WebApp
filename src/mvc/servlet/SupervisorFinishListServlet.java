...
public class SupervisorFinishListServlet extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String contentPage ="/jsp/supervisorFinishList.jsp";
		...
		//set page
		try {
			reportamount = DAOFactory.getIReportDAOInstance().getAmountSupervisorFinishList(roleid);	//get amount of records the role's group have	
		} catch (Exception e) {
			e.printStackTrace();
		}
		int pagesize = 17;
		...
		//set report
		try {
			backinfo = DAOFactory.getIReportDAOInstance().supervisorFinishListReport(page,roleid);
		} catch (Exception e) {
			e.printStackTrace();
		}
		//jump to page
