				...
				<tr align="center">
					 <th colspan="6"> 
					 	<%if(pagex.currentPage>1){ %>
        					<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.previousPage %>">Previous Page</a> 
        				<%}else{%>
        					<a>Previous Page</a>
        				<%} %>
        				<%if(pagex.getTotalPage()<6) {
        					for(int indexx=1;indexx<=pagex.getTotalPage();indexx++){%>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=indexx%>"><%=indexx %></a>
        				<% }
        				}
        				else{
        				...
        				<%  	}%>
        						<a>...</a>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.getTotalPage()%>"><%=pagex.getTotalPage()%></a>
        				<%	}
        				...
        				<%	}
        						...
        						<a>...</a>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.getTotalPage()-3%>"><%=pagex.getTotalPage()-3%></a>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.getTotalPage()-2%>"><%=pagex.getTotalPage()-2%></a>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.getTotalPage()-1%>"><%=pagex.getTotalPage()-1%></a>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.getTotalPage()%>"><%=pagex.getTotalPage()%></a>
        				<%	}
        					else{%>
        						<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=1">1</a>
        						<a>...</a>
        				...
        				<%if(pagex.nextPage<=pagex.getTotalPage()) {%>
        					<a href="jsp/SupervisorFinishListServlet?roleid=<%=request.getAttribute("roleid")%>&pageindex=<%=pagex.nextPage%>">Next Page</a> 
        				<%}else{%>
        					<a>Next Page</a>
        				<%} %>
					 </th>
