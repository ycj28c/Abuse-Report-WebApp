				...
				<tr align="center">
					 <th colspan="9"> 
					 	<%if(pagex.currentPage>1){ %>
        					<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.previousPage %>">Previous Page</a> 
        				<%}else{%>
        					<a>Previous Page</a>
        				<%} %>
        				<%if(pagex.getTotalPage()<6) {
        					for(int indexx=1;indexx<=pagex.getTotalPage();indexx++){%>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=indexx%>"><%=indexx %></a>
        				<% }
        				}
        				else{
        				...
        				<%  	}%>
        						<a>...</a>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.getTotalPage()%>"><%=pagex.getTotalPage()%></a>
        				<%	}
        				...
        				<%	}
        						...
        						<a href="jsp/HRCListReportServlet?pageindex=1">1</a>
        						<a>...</a>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.getTotalPage()-3%>"><%=pagex.getTotalPage()-3%></a>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.getTotalPage()-2%>"><%=pagex.getTotalPage()-2%></a>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.getTotalPage()-1%>"><%=pagex.getTotalPage()-1%></a>
        						<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.getTotalPage()%>"><%=pagex.getTotalPage()%></a>
        				<%	}
        					else{%>
        						<a href="jsp/HRCListReportServlet?pageindex=1">1</a>
        				...
        				<%if(pagex.nextPage<=pagex.getTotalPage()) {%>
        					<a href="jsp/HRCListReportServlet?pageindex=<%=pagex.nextPage%>">Next Page</a> 
        				<%}else{%>
        					<a>Next Page</a>
        				<%} %>
					 </th>
